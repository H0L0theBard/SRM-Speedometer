...
void function SRM_CreateSpeedometer()
{
	...
	RuiSetFloat2( file.speedometer, "msgPos",<GetConVarFloat("srm_speedometer_position_x"),GetConVarFloat("srm_speedometer_position_y"),0.0>)
	...
	RuiSetFloat2( file.speedometerUnit, "msgPos",<GetConVarFloat("srm_speedometer_position_x"),GetConVarFloat("srm_speedometer_position_y") + 0.04,0.0>)
...
void function SRM_SpeedometerUpdate()
{
	...
	while (true)
	{
		...
		switch ( GetConVarInt("srm_speedometer_axismode") )
		{
			...
			break
			// Z
			case 1: speedometerVelocityVector = < 0.0, 0.0, speedometerVelocityVector.z >
			break
			// XYZ
			case 2: // redundant since the other cases just eliminate axes to ignore
			break
...
{
	array<string> colorslow = split( GetConVarString( "srm_speedometer_color_slow" ), " " )
	array<string> colorfast = split( GetConVarString( "srm_speedometer_color_fast" ), " " )

	// there's probably a much better way of doing this by storing the convars as an actual vector but idk how :[
	return <
	GraphCapped( speedometerVelocity, 300.0, 1000.0, float(colorslow[0]), float(colorfast[0]) ),
	GraphCapped( speedometerVelocity, 300.0, 1000.0, float(colorslow[1]), float(colorfast[1]) ),
	GraphCapped( speedometerVelocity, 300.0, 1000.0, float(colorslow[2]), float(colorfast[2]) )
	>
}
